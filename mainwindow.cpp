...
void MainWindow::addToPlots(TickData tick_data)
{
    ...
    qint64 liczba_probek = (double)ui->spinBox_symulacja_okno_obserwacji->value() / ((double)interwal_symulacji / 1000.0);
    ...
    for(QList<QPointF>::iterator point = lista_wartosc_zadana->begin(); point != lista_wartosc_zadana->end(); point = std::next<>(point))
    {
    ...
    for(QList<QPointF>::iterator point = lista_wartosc_regulowana->begin(); point != lista_wartosc_regulowana->end(); point = std::next<>(point))
    {
    ...
    qreal range_width_wartosc_zadana_i_regulowana = (std::max(max_wartosc_regulowana, max_wartosc_zadana) - std::min(min_wartosc_regulowana, min_wartosc_zadana)) * 0.1;

    chart_wartosc_zadana_i_regulowana->axes(Qt::Vertical).at(0)->setRange(std::min(min_wartosc_regulowana, min_wartosc_zadana) - range_width_wartosc_zadana_i_regulowana,
                                                                          std::max(max_wartosc_regulowana, max_wartosc_zadana) + range_width_wartosc_zadana_i_regulowana);


    qreal max_wszystko = std::max(std::max(max_uchyb, max_sterowanie), std::max(max_wartosc_zadana, max_wartosc_regulowana));
    qreal min_wszystko = std::min(std::min(min_uchyb, min_sterowanie), std::min(min_wartosc_zadana, min_wartosc_regulowana));
    qreal range_width_wszystko = (max_wszystko - min_wszystko) * 0.1;

    chart_wszystko->axes(Qt::Vertical).at(0)->setRange(min_wszystko - range_width_wszystko, max_wszystko + range_width_wszystko);
...
{
    State::getInstance().getGeneratorProstokatny().setAmplitude(ui->doubleSpinBox_generator_amplituda->value());
    State::getInstance().getGeneratorSinusoida().setAmplitude(ui->doubleSpinBox_generator_amplituda->value());
}
void MainWindow::on_verticalSlider_generator_amplituda_sliderReleased()
{
    State::getInstance().getGeneratorProstokatny().setAmplitude((double)ui->verticalSlider_generator_amplituda->value() / 100.0);
    State::getInstance().getGeneratorSinusoida().setAmplitude((double)ui->verticalSlider_generator_amplituda->value() / 100.0);
}
...
{
    State::getInstance().getGeneratorProstokatny().setSamplesPerCycle(ui->spinBox_generator_okres->value());
    State::getInstance().getGeneratorSinusoida().setSamplesPerCycle(ui->spinBox_generator_okres->value());
}
void MainWindow::on_horizontalSlider_generator_okres_sliderReleased()
{
    State::getInstance().getGeneratorProstokatny().setSamplesPerCycle(ui->horizontalSlider_generator_okres->value());
    State::getInstance().getGeneratorSinusoida().setSamplesPerCycle(ui->horizontalSlider_generator_okres->value());
}
...
{
    State::getInstance().getGeneratorProstokatny().setDutyCycle((double)ui->spinBox_generator_wypelnienie->value() / 100.0);
}
void MainWindow::on_horizontalSlider_generator_wypelnienie_sliderReleased()
{
    State::getInstance().getGeneratorProstokatny().setDutyCycle((double)ui->horizontalSlider_generator_wypelnienie->value() / 100.0);
}
